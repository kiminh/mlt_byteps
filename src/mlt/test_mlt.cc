 ...
 private:
  ...
  inline Node FindNode(int rank) {
    auto it = std::find_if(nodes_.begin(), nodes_.end(),
                           [&rank](const Node& n) { return n.rank == rank; });
    CHECK(it != nodes_.end()) << "could not find rank: " << rank;
    return *it;
  }

  inline std::string Hello(const Node& node) {
    return StringHelper::FormatString("Hello, %s:%d!", node.host.c_str(), node.port);
  }
...
int TestMLTApp::Startup() {
  ...
  for (const Node& node : nodes_) {
    ...
    mlt_comm->AddConnection(rank, node.host, node.port);
  }
