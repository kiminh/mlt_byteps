...
#define CHECK_BINARY_OP(name, op, x, y)                               \
  if (prism::LogCheckError _check_err = prism::LogCheck##name(x, y))    \
    prism::LogMessageFatal(__FILE__, __LINE__).sstream()                \
...
#define CHECK_NOTNULL(x)                                             \
  ((x) == NULL ? prism::LogMessageFatal(__FILE__, __LINE__).sstream() \
                     << "Check  notnull: " #x << ' ',                \
   (x) : (x))  // NOLINT(*)
// Debug-only checking.
#ifdef NDEBUG
...
#define LOG_TRACE prism::LogMessage(__FILE__, __LINE__, prism::LogLevel::TRACE)
#define LOG_DEBUG prism::LogMessage(__FILE__, __LINE__, prism::LogLevel::DEBUG)
#define LOG_INFO prism::LogMessage(__FILE__, __LINE__, prism::LogLevel::INFO)
#define LOG_WARNING prism::LogMessage(__FILE__, __LINE__, prism::LogLevel::WARNING)
#define LOG_ERROR prism::LogMessage(__FILE__, __LINE__, prism::LogLevel::ERROR)
#define LOG_FATAL prism::LogMessageFatal(__FILE__, __LINE__)
#define LOG_QFATAL LOG_FATAL

// Poor man version of VLOG
#define VLOG(x) LOG_INFO.sstream()

#define LOG(severity) LOG_##severity.sstream()
#define LG LOG_INFO.sstream()
...
#define PLOG_TRACE prism::ErrnoLogMessage(__FILE__, __LINE__, prism::LogLevel::TRACE)
#define PLOG_DEBUG prism::ErrnoLogMessage(__FILE__, __LINE__, prism::LogLevel::DEBUG)
#define PLOG_INFO prism::ErrnoLogMessage(__FILE__, __LINE__, prism::LogLevel::INFO)
#define PLOG_WARNING prism::ErrnoLogMessage(__FILE__, __LINE__, prism::LogLevel::WARNING)
#define PLOG_ERROR prism::ErrnoLogMessage(__FILE__, __LINE__, prism::LogLevel::ERROR)
#define PLOG_FATAL prism::ErrnoLogMessageFatal(__FILE__, __LINE__)
#define PLOG_QFATAL PLOG_FATAL

#define PLOG(severity) PLOG_##severity.sstream()
...
#define PCHECK(condition)                               \
  if (!(condition))                                     \
    prism::ErrnoLogMessageFatal(__FILE__, __LINE__).sstream() \
...
class DateLogger {
 public:
  ...
  const char* FullTimeData() {
    ...
    auto micros_remainder = duration_cast<microseconds>(duration - seconds);

    const size_t time_buffer_size = 30;
    char time_buffer[time_buffer_size];
    strftime(time_buffer, time_buffer_size, "%Y-%m-%d %H:%M:%S",
             localtime(&as_time_t));
    snprintf(buffer_, sizeof(buffer_), "%s.%06ld", time_buffer,
             micros_remainder.count());
    return buffer_;
  }
...
static const char* severity_msg[] = {"TRACE",   "DEBUG", "INFO",
                                     "WARNING", "ERROR", "FATAL"};
...
static inline LogLevel ParseLogLevelStr(const char* env_var_val) {
...
static inline LogLevel MinLogLevelFromEnv() {
  const char* env_var_val = getenv("PRISM_LOG_LEVEL");
...
static inline LogLevel GetMinLogLevel() {
...
class LogMessage {
...
#else
        log_stream_(severity <= LogLevel::INFO ? std::cout : std::cerr),
#endif
        severity_(severity)
  {
    if (!ShouldPrint()) return;
    preserved_errno_ = errno;
    sstream() << "[" << pretty_date_.FullTimeData() << "] "
             << severity_msg[static_cast<int>(severity)] << " " << file << ":"
             << line << ": ";
  }
  ~LogMessage() {
    ...
    sstream() << "\n";
    log_stream_ << sstream().str();
  }
  std::ostream& stream() { return log_stream_; }

  std::ostringstream& sstream() { return os_; }
 ...
 protected:
  bool ShouldPrint() {
    static LogLevel min_log_level = MinLogLevelFromEnv();
    return severity_ >= min_log_level;
  }
...
class ErrnoLogMessage : public LogMessage {
 public:
  ErrnoLogMessage(const char* file, int line, LogLevel severity)
  ...
  ~ErrnoLogMessage() {
    ...
    if (ShouldPrint()) {
      sstream() << ": " << StrError(preserved_errno()) << " ["
                << preserved_errno() << "]";
      stream() << sstream().str();
    }
  }
...
class LogMessageFatal : public LogMessage {
 public:
  LogMessageFatal(const char* file, int line)
      : LogMessage(file, line, prism::LogLevel::FATAL) {}
  ~LogMessageFatal() {
    sstream() << "\n\n" << StackTrace() << "\n";
    stream() << sstream().str();
    abort();
...
class ErrnoLogMessageFatal : public ErrnoLogMessage {
 public:
  ErrnoLogMessageFatal(const char* file, int line)
      : ErrnoLogMessage(file, line, prism::LogLevel::FATAL) {}
...
class LogMessageFatal {
 public:
  LogMessageFatal(const char* file, int line) {
    log_stream_ << "[" << pretty_date_.FullTimeData() << "] " << file << ":"
                << line << ": ";
  }
